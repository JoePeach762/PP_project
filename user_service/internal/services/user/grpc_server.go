...
func (s *GRPCServer) GetUsers(ctx context.Context, req *users_api.GetUsersRequest) (*users_api.GetUsersResponse, error) {
	...
	for _, u := range users {
		pbUsers = append(pbUsers, &pbmodels.UserInfo{
			...
			CurrentCalories: u.CurrentCalories,
			CurrentProteins: u.CurrentProteins,
			CurrentFats:     u.CurrentFats,
			CurrentCarbs:    u.CurrentCarbs,
			TargetCalories:  u.TargetCalories,
			TargetProteins:  u.TargetProteins,
			TargetFats:      u.TargetFats,
			TargetCarbs:     u.TargetCarbs,
		})
	}
