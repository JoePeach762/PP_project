...
func TestNewUserService_NormalizesAllowedSexes(t *testing.T) {
	...
	if len(service.allowedSexes) != 2 {
		t.Fatalf("unexpected allowed sexes count: %d", len(service.allowedSexes))
	}

	if _, ok := service.allowedSexes["male"]; !ok {
		t.Fatalf("expected male to be normalized into allowedSexes")
	}

	if _, ok := service.allowedSexes["female"]; !ok {
		t.Fatalf("expected female to be normalized into allowedSexes")
	}
}
